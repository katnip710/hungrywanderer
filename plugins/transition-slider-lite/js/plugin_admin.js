...
(function($) {
    $(document).ready(function() {
        ...
        $form.submit(function(e) {
            ...
            var slider = JSON.stringify(options);
        ...
        addOption("general-settings", "shadow", "dropdown", "Slider shadow", "effect2", ["effect1", "effect2", "effect3", "effect4", "effect5", "effect6", "off"], "");
        ...
        addOption("size", "ratio", "text", "Responsive ratio (width / height)", "", "", "isSubitem");
        ...
        addOption("buttons", "buttons.pauseVisible", "checkbox", "Pause button", true, "", "", "");
        addOption("buttons", "buttons.muteVisible", "checkbox", "Mute button", true, "", "", "");

        addOption("arrows", "navigation", "textOnly", "Arrows", "", "", "hasSubitem", "");
        addOption("arrows", "navigation.enable", "checkbox", "Enable", true, "", "isSubitem", "");
        addOption("arrows", "navigation.style", "dropdown", "Style", "effect4", ["effect1", "effect2", "effect3", "effect4", "effect5", "effect6", "effect7", "effect8", "effect9", "effect10"], "isSubitem", "hasPreview");
        ...
        addOption("pagination", "pagination.style", "dropdown", "Style", "effect2", ["effect1", "effect2", "effect3", "effect4", "effect5", "effect6"], "isSubitem", "");
        ...
        addOption("loading", "loading.backgroundColor", "color", "Background color", "#000", "", "");
        addOption("loading", "loading.textColor", "color", "Text color", "#FFF", "", "");
        ...
        $(".size-desktop").click(function(){
            $("#slider-preview-container").css("width", "100%")
            $(window).trigger("resize")
        })

        $(".size-tablet").click(function(){
            $("#slider-preview-container").css("width", "1023px")
             $(window).trigger("resize")
        })

        $(".size-mobile").click(function(){
            $("#slider-preview-container").css("width", "767px")
             $(window).trigger("resize")
        })
     ...
     -------------------- */
        ...
        var transitionOptions = {
            ...
            crossfade: {
                direction: [{ name: "In", value: "in" }, { name: "Out", value: "out" }],
            },
     ...
     -------------------- */
        ...
        function renderLayers() {
            ...
            layerRenderer.render(options.slides[currentSlide].elements);
        }

        function updateCurrentElement(settingName) {
            layerRenderer.updateElement(currentElement, settingName);
        }
        ...
        function updateElementOffset(offset) {
            selectedElements.forEach(function(index) {
                ...
                el.position.offsetX = Math.round(Number(el.position.offsetX || 0));
                el.position.offsetY = Math.round(Number(el.position.offsetY || 0));
                el.position.offsetX += o.x;
                el.position.offsetY += o.y;
                layerRenderer.updateElement(index, 'position');
                updateElementSetting("position.offsetX", el.position.offsetX)
                updateElementSetting("position.offsetY", el.position.offsetY)
            });
        }
        ...
        function resizeLayers() {
            ...
            (lw = options.slides[currentSlide].layerWidth || 1920), (lh = options.slides[currentSlide].layerHeight || 950);

            layerRenderer.resize(sw, sh, lw, lh);
        }
        ...
        $elementSettings.find("input").on("change keyup paste", function() {
        ...
        $elementSettingsHover.find("input").on("change keyup paste", function() {
        ...
        function onElementSetingChange(target, hover) {
            if (target.name) {
                ...
                //reset offset to 0 if position has changed
            ...
            updateCurrentElement(target.name);
        }

        $('textarea[name="customCSS"]').bind("change paste", function() {
            var el = getCurrentElement();
        ...
        function addNavigatorElement(obj) {
            ...
            $el.attr("id", numItems).click(function(e) {
                ...
                onLayerClick($(this).index(), e.shiftKey);
            });
        ...
        function updateElementSettings(obj, hover) {
            ...
            // $(".element-tab-content a").trigger("click");
        }
        ...
        var $layerListPopup = $(".layer-list-popup")
            .draggable({
                ...
                containment: ".slider-preview-area"
            })
            .resizable({
                autoHide: true,
                minWidth: 200,
        ...
        $(".element-template").click(function(e) {
            ...
            var el = getCurrentElement();
            var style = jQuery(this).css(["color", "backgroundColor", "fontSize", "fontWeight", "paddingTop", "paddingLeft", "paddingRight", "paddingBottom", "borderWidth", "borderColor", "borderStyle", "borderRadius"]);

            el.textColor = style.color;
            el.backgroundColor = style.backgroundColor;
            el.fontSize = style.fontSize.replace("px", "");
            el.fontWeight = style.fontWeight;
            el.paddingTop = style.paddingTop.replace("px", "");
            el.paddingLeft = style.paddingLeft.replace("px", "");
            el.paddingRight = style.paddingRight.replace("px", "");
            el.paddingBottom = style.paddingBottom.replace("px", "");
            el.borderWidth = style.borderWidth.replace("px", "");
            el.borderColor = style.borderColor;
            el.borderStyle = style.borderStyle;
            el.borderRadius = style.borderRadius.replace("px", "");

            onLayerClick(currentElement);
        });
        ...
        function deleteLayerElements() {
            ...
            //sort and reverse array on indexes
        ...
        window.addEventListener("contextmenu", function(e) {
            if ($(e.target).hasClass("layer-container")) {
                e.preventDefault();
                var origin = {
